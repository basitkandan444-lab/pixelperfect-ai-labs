...
interface CachedMeta { url: string; size: number; ts: number }
...
async function readMeta(cache: Cache): Promise<CachedMeta[]> {
  ...
  if (!res) return [];
  try { return (await res.json()) as CachedMeta[]; } catch { return []; }
}
...
async function evictIfOver(cache: Cache) {
  ...
  for (const m of meta.reverse()) {
    if (total <= MAX_BYTES) { kept.push(m); continue; }
